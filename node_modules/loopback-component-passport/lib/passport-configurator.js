...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var defaultCallback = function (req, res, next) {
    ...
    passport.authenticate(name, _.defaults({session: session},
      ...
      if (err) {
        // console.log("#####################");
        // console.log("########## ERROR DENTRO DE MODELO USER ###########");
        // console.log(err);
        // console.log("#####################");
        // return res.status(401).json(err);
        return res.redirect('/#!/?credentials=' + encodeURIComponent('incorrect'));
      }
      if (!user) {
        if (!!options.json) {
          console.log('ERROR de LOGIN --->', info);
          // return res.status(401).json(info)
          // console.log("#####################");
          // console.log(info);
          // console.log("#####################");

          return res.redirect('/#!/?credentials=' + encodeURIComponent('missing'));
        }
      ...
      if (info && info.accessToken) {
        ...
        if (!!options.json) {
          ...
          //   userId: user.id
          // });
          return res.redirect('/#!/authentication/' + info.accessToken.id + '/' +  user.id);
        }
      }
